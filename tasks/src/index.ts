...
export default definePluginEntry({
  id: "tasks",
  name: "Tasks",
  description: "Manage local tasks, projects, priorities, and recurring work inside OpenClaw",
  register(api) {
    ...
    api.registerTool({
      name: "tasks_list",
      label: "List tasks",
      description: "List tasks by view, status, project, tag, priority, due window, or search query",
      parameters: Type.Object({
        view: Type.Optional(
          Type.Union([
            Type.Literal("all"),
      ...
      async execute(_id, params) {
        ...
        return toolTextResult(formatTaskList("Matching tasks", tasks), {
          status: "ok",
          count: tasks.length,
          tasks,
        });
    ...
    api.registerTool({
      name: "tasks_agenda",
      label: "Task agenda",
      description: "Summarize overdue, due soon, scheduled, and backlog tasks for planning",
      parameters: Type.Object({
