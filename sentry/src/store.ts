...
export class SentryStore {
  ...
  constructor(config: SentryPluginConfig = {}, fetchImpl: FetchLike = fetch) {
  ...
  async listOrganizations(input: ListOrganizationsInput = {}): Promise<SentryOrganization[]> {
    ...
    const response = await this.request<Array<Record<string, unknown>>>(
      `/organizations/${query.size ? `?${query.toString()}` : ""}`,
    );

    return response.map(mapOrganization).slice(0, normalizeLimit(input.limit));
  }

  async listProjects(input: ListProjectsInput = {}): Promise<SentryProject[]> {
    const organizationSlug = this.resolveOrganizationSlug(input.organizationSlug);
    const response = await this.request<Array<Record<string, unknown>>>(
      `/organizations/${encodeURIComponent(organizationSlug)}/projects/`,
    );

    return response.map(mapProject).slice(0, normalizeLimit(input.limit));
  }

  async listIssues(input: ListIssuesInput = {}): Promise<SentryIssue[]> {
    const organizationSlug = this.resolveOrganizationSlug(input.organizationSlug);
    ...
    const response = await this.request<Array<Record<string, unknown>>>(
      `/organizations/${encodeURIComponent(organizationSlug)}/issues/${query.size ? `?${query.toString()}` : ""}`,
    );

    return response.map(mapIssue).slice(0, normalizeLimit(input.limit));
  }

  async getIssue(input: GetIssueInput): Promise<SentryIssue> {
    const organizationSlug = this.resolveOrganizationSlug(input.organizationSlug);
  ...
  private resolveOrganizationSlug(organizationSlug?: string): string {
...
function mapOrganization(value: Record<string, unknown>): SentryOrganization {
  const status = value.status;
  const statusId =
    status && typeof status === "object" && "id" in status ? asString(status.id) : asString(status);
  const links = value.links;
  const organizationUrl =
    links && typeof links === "object" && "organizationUrl" in links
...
function mapProject(value: Record<string, unknown>): SentryProject {
  const team = value.team;
  const teamSlug =
    team && typeof team === "object" && "slug" in team ? asString(team.slug) : undefined;
...
function mapIssue(value: Record<string, unknown>): SentryIssue {
  const metadata = value.metadata;
  const metadataTitle =
    metadata && typeof metadata === "object" && "title" in metadata ? asString(metadata.title) : undefined;
  const project = value.project;
  const projectSlug =
    project && typeof project === "object" && "slug" in project ? asString(project.slug) : undefined;
  const assignedTo = value.assignedTo;
  const assignedName =
    assignedTo && typeof assignedTo === "object" && "name" in assignedTo
