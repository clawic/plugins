export function formatResearch(research, activeResearchId) {
    const parts = [
...
export function formatResearchList(label, researches, activeResearchId) {
    if (!researches.length) {
        return `${label}: none`;
    }
    return `${label}:\n${researches.map((research) => `- ${formatResearch(research, activeResearchId)}`).join("\n")}`;
}
export function formatResearchDetail(research, activeResearchId) {
    const sections = [
...
export function formatResearchResource(resource) {
...
export function formatResearchSynthesis(synthesis) {
    const parts = [
